...
int rule() {
	...
	puts("ルールの説明");
	puts("まず石の数を設定します");
	puts("あなたとコンピュータは設定した医師の中から1~3個までの石を順番に取れます");
	puts("最後の１個の石を取ったほうが負けです");
	puts("先攻後攻はランダムです");
...
int RockSet() {
	...
	while (1) {
		puts("石の数を設定します。");
		puts("12個(1)、24個(2)、32個(3)12~32のランダム(4)自由(5)の中から選び、\n選んだら()の中の数字を入力してください");
